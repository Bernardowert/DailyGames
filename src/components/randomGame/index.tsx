...
export function RandomGame({id,title,image_url}: GameInfo){
    return(
        <Link href={`/game/${id}`} className="relative h-96 block rounded-lg overflow-hidden group">
           <Image
            src={image_url || ''}
            alt={title || ''}
            title={title}
            fill
            loading="eager"
            className="!static object-cover transition-all ease-linear filter group-focus:brightness-50 group:focus:scale-105  group-hover:brightness-50 group-hover:scale-105"
           />
